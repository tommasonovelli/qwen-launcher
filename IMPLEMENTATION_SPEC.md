...
mode whose route declares the image modality. The overlay also disables the model-backed session
titler, so a turn issues exactly one completion against the single calibrated slot while upstream's
own fallback still names the session from its first prompt.
...
permission preset lowered to `read-only` rather than inherited, and the key llama-server ignores
passed by environment-variable reference so no secret reaches disk. An inherited `DEEPSEEK_API_KEY`
is removed, because the routes that would use it are disabled and a key left in the environment
would quietly re-enable a hosted endpoint. The upstream licence ships in
...
and warns of compatibility-breaking changes, the pinned version is a pre-release, and Windows was not
exercised. None of those is claimed as resolved, and no version, tag, or release is authorized by
D-097.
